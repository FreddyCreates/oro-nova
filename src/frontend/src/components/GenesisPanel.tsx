/**
 * GenesisPanel.tsx — Genesis corpus field status.
 * Wired to real backend: getGenesisAnchor + getLawRegistry.
 * Zero-exposure wall: law indices only, NO doctrine names.
 * Kernel Compression Protocol v1.
 */
import { useEffect, useRef, useState } from "react";
import type { LawRegistryEntry } from "../backend.d.ts";
import {
  HEARTBEAT_MS,
  OMNIS,
  PHI,
...
// ─── Cluster φ-decay ladder — indices only, no doctrine names ─────────────────
const PHI_WEIGHTS = [
  1.0,
  PHI_INV,
  PHI_INV2,
...
type GenesisAnchor = {
  tzolkinDay?: bigint | number;
  haabDay?: bigint | number;
  venusPhase?: number;
  precessionalPos?: number;
...
// ─── Circular cluster visualization (12 nodes, phi-weighted opacity) ──────────
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const pulse =
          1 +
          Math.cos(Math.PI * heartPhase * PHI_INV + t * 0.5 + i * 0.2) ** 2 *
            PHI_INV4;
        const opacity = Math.min(w * pulse, 1);
        const law = laws[i];
        const isAnchored = law?.genesisAnchored ?? false;
        ...
        ctx.fillStyle = emerged
          ? `oklch(${50 + w * 30}% 0.18 75 / ${opacity})`
          : `oklch(${40 + w * 25}% 0.12 195 / ${opacity})`;
        ctx.fill();
...
export function GenesisPanel() {
  ...
  const {
    ...
    async () => {
      ...
      return actor.getGenesisAnchor() as Promise<GenesisAnchor>;
    },
    PHI3_SECOND_MS,
    [actor, isFetching],
  );
  ...
  // ── Poll field state for heartPhase ─────────────────────────────────────
  ...
  const emerged = fieldState?.emergenceState ?? false;
  const panelColor = emerged ? "oklch(80% 0.18 75)" : "oklch(60% 0.12 195)";
  const panelBorder = emerged
    ? "oklch(40% 0.12 75 / 0.4)"
    : "oklch(25% 0.08 195 / 0.4)";
  const lawList = laws ?? [];
  ...
  const anchorFields = anchor
    ? [
        ...
        },
        {
          label: "PRECESSIONAL",
          value:
            anchor.precessionalPos != null
              ? anchor.precessionalPos.toFixed(6)
              : "—",
        },
      ]
    : [];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          GENESIS ANCHOR — PHASE COORDINATES
        </div>
        {anchorLoading && <SkeletonRow />}
        {!anchorLoading && anchorError && <ErrorState />}
        {!anchorLoading && anchor && (
          <div
            style={{
              display: "grid",
        ...
        {!anchorLoading && !anchor && !anchorError && (
          <div
            style={{
              fontSize: 10,
        ...
        >
          ...
          >
            ∞ ← Precessional phase lock — permanent amplitude
          </span>
          <span
            style={{
              fontSize: 14,
      ...
      >
        ...
        >
          GENESIS QUATERNION — Φ/2π × 1/√Φ × 1/τ × Φ⁻²
        </div>
      ...
      {/* 12 cluster nodes — circular ring + phi-weighted list */}
      <div
        style={{
          background: "oklch(6% 0 0)",
      ...
      >
        ...
        >
          12 LAW CLUSTERS — Φ⁻ⁿ DECAY LADDER (INDICES ONLY)
        </div>
        {lawsLoading && <SkeletonRow />}
        {!lawsLoading && lawsError && <ErrorState />}
        {!lawsLoading && (
          <ClusterRing
            laws={lawList}
            ...
            return (
              ...
              >
                ...
                {law && (
                  ...
                  >
                    h:{law.lawHash.toString(16).slice(-4)}
                  </span>
                )}
              </div>
            );
      ...
      >
        ...
        >
          BEAT Φ⁰: {HEARTBEAT_MS}ms | Φ¹: {Math.round(HEARTBEAT_MS * PHI)}ms |
          OMNIS: {OMNIS.toFixed(4)}
        </span>
        <span
          style={{
            fontFamily: "JetBrains Mono, monospace",
