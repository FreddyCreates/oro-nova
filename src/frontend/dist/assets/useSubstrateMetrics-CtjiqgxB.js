...
import { q as PHI2_SECOND_MS } from "./index-DQuwpKqn.js";
import { a as asFullBackend } from "./extendedBackend-Cw0NHKI2.js";
const POLL_MS = PHI2_SECOND_MS;
function computeBrainRegions(intero, emergence, frb, snap, identity) {
  ...
  return [
    ...
    {
      name: "Hippocampus",
      activation: emergence.memoryEffectScore * 0.7 + frb.encodingIntensity * 0.3
    },
    {
      name: "Amygdala",
    ...
    },
    {
      ...
      activation: pred.predictionError * 0.6 + Math.min(intero.overloadIndex * 0.4, 1)
    },
    {
    ...
    },
    {
      ...
      activation: intero.stabilityScore * 0.6 + (1 - intero.internalNoise) * 0.4
    },
    {
      name: "ZonaIncerta",
    ...
    {
      name: "TemporalCortex",
      activation: emergence.animalTraitStrength * 0.5 + emergence.continuityDepth * 0.5
    },
    {
      name: "DorsalACC",
    ...
    {
      name: "CA1",
      activation: emergence.continuityDepth * 0.6 + emergence.memoryEffectScore * 0.4
    },
    {
      name: "CA3",
...
function useSubstrateMetrics() {
  ...
  const synthReport = synthesisReport == null ? void 0 : synthesisReport[0];
  ...
  const meanActivation = brainRegions.reduce((s, r) => s + r.activation, 0) / brainRegions.length;
  ...
  const cardioInfluenceRate = Math.abs(hrProxy - 72) / 94 * intero.arousalLevel;
  const ansInfluenceRate = Math.abs(ansBalance) * emergence.embodimentCouplingScore;
  const recoveryResponseQuality = intero.stabilityScore * (1 - intero.overloadIndex);
  const cardioANS = {
    hrProxy,
    hrvProxy,
    collapseRisk,
  ...
  const sensoryToSalienceBoost = sensoryRelevance * 0.456 + (1 - uncertaintyBurden) * 0.17;
  const sensoryToWMGatePressure = (1 - uncertaintyBurden) * 0.17;
  const degradationUnderLoad = intero.overloadIndex * 0.15;
  const overallInfluenceRate = sensoryToSalienceBoost * emergence.embodimentCouplingScore;
  const sensoryCoupling = {
    sensoryRelevance,
    uncertaintyBurden,
  ...
  const antiFakeGates = [
    {
      ...
      passing: ((synthReport == null ? void 0 : synthReport.overallSystemHealth) ?? 0) > 0.1
    },
    {
      id: "inject_conclusion_blocked",
      label: "inject_conclusion_blocked",
      passing: ((synthReport == null ? void 0 : synthReport.realityConfidence) ?? 0) > 0.05
    },
    {
    ...
    },
    {
      ...
      passing: (((_a = snap.chosenAction.source) == null ? void 0 : _a.includes("arbitration")) ?? false) || emergence.intelligenceIndex > 0.3
    },
    {
  ...
  const antiFakeGateScore = antiFakeGates.filter((g) => g.passing).length / antiFakeGates.length;
  const STABILITY = (identity.coherence * 0.7 + (1 - intero.internalNoise) * 0.3) * 100;
  const SELECTIVITY = (frb.coordinationQuality * 0.6 + (1 - intero.overloadIndex) * 0.4) * 100;
  const RECURRENCE = Math.min(Number(entityStatus.cycleCount) / 500, 1) * emergence.continuityDepth * 100;
  const parasympTone = intero.stabilityScore * 0.7 + intero.recoveryPressure * 0.3;
  const sympathTone = intero.arousalLevel * 0.8 + intero.overloadIndex * 0.2;
  const REGULATION = (Math.max(0, 1 - Math.abs(parasympTone - sympathTone)) * 0.6 + recoveryResponseQuality * 0.4) * 100;
  ...
  if (frb.coordinationQuality < 0.6)
    recs.push({
      ...
      module: "core-circuit",
      rationale: "Coordination quality below 0.6 indicates inter-regional synchrony deficit.",
  ...
  if (Math.abs(ansBalance) > 0.3)
    recs.push({
      ...
      module: "regulation",
      rationale: "ANS balance magnitude exceeds 0.3, creating systemic regulation asymmetry.",
      suggestedAction: "Reduce parasympathetic re-engagement threshold from 0.7 to 0.55.",
      status: "PENDING"
    });
  if (snap.predictionErrorSignal.predictionError > 0.4)
    recs.push({
      ...
      suggestedAction: "Wire prediction-error bridge weight 0.8 on mismatch > 0.4.",
      status: "PENDING"
    });
  if (snap.salienceMap.globalUrgency > 0.7)
    recs.push({
      ...
      suggestedAction: "Add urgency ceiling at 0.75 before full-dense ignition.",
      status: "PENDING"
    });
  if (cardioInfluenceRate < 0.3)
    recs.push({
      ...
      module: "cardio-ans",
      rationale: "Heart rate proxy influence on action threshold is below optimal.",
  ...
  if (emergence.memoryEffectScore < 0.4)
    recs.push({
      ...
      suggestedAction: "Implement LTD penalty: after 3 failures W(pathway) *= 0.85.",
      status: "PENDING"
    });
  if (emergence.adaptationValidity < 0.5)
    recs.push({
      ...
      suggestedAction: "Replace fixed rate with alpha(t) = alpha0 * stability * (1 - fatigue).",
      status: "PENDING"
    });
  if (emergence.continuityDepth < 0.5)
    recs.push({
      ...
      suggestedAction: "Implement semantic clustering with cosine similarity threshold 0.7.",
      status: "PENDING"
    });
  recs.push({
    id: "harden_binding",
  ...
  if (intero.overloadIndex > 0.6)
    recs.push({
      ...
      suggestedAction: "Implement dynamic slot allocation: reduce slot count at overload > 0.6.",
      status: "PENDING"
    });
  ...
  const aiIntelligenceScore = Math.round(
    (overallScore / 100 * 0.4 + thoughtCoherence * 0.3 + emergence.intelligenceIndex * 0.3) * 100
  );
  const researchScore = Math.round(
    (overallScore / 100 * 0.5 + emergence.continuityDepth * 0.3 + emergence.adaptationValidity * 0.2) * 100
  );
  const militaryPoliticalScore = Math.round(
    (frb.coordinationQuality * 0.5 + emergence.intelligenceIndex * 0.3 + emergence.adaptationValidity * 0.2) * 100
  );
  const commercializationScore = aiIntelligenceScore > 60 && researchScore > 60 ? 30 : 0;
  ...
  const stdpBase = 0.7 + emergence.adaptationValidity * 0.3 + snap.actionOutcome.rewardSignal * 0.1;
  const stdpWeights = [
  ...
  const arousalMode = intero.arousalLevel < 0.2 ? "calm" : intero.arousalLevel < 0.5 ? "alert" : intero.arousalLevel < 0.8 ? "activated" : "overloaded";
  const interoInfluenceRate = emergence.embodimentCouplingScore * 100;
  const overloadResponseQuality = (1 - intero.overloadIndex) * intero.stabilityScore * 100;
  const recoveryResponseQualityPct = (1 - intero.globalFatigue) * intero.stabilityScore * 100;
  const readinessScore = overallScore;
  const activeCouplings = Math.floor(emergence.embodimentCouplingScore * 5);
  const avgPathwayStrength = frb.coordinationQuality * 0.5 + emergence.adaptationValidity * 0.5;
